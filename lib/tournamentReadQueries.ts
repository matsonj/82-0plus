...
// (/api/tournament/{bracket,team,lookup}). These run on the read-only Postgres pool
// (lib/oltpReadDb) — a separate, lower-privilege connection (DATABASE_URL_RO) that
// never runs DDL or writes, keeping the PIN/auth table off the wider read surface.
// Unlike the old MotherDuck share, reads hit the same always-on database the
// writers use, so there is no replication lag.
//
...
// slot order. Mirrors the signature in listDailyResults so the dedup here matches
// the champion-credit logic there. `arr` is a jsonb expression yielding the player
// array (teams.roster_display nests under -> 'roster'; daily_results.roster_json
// is already a bare array).
function rosterSig(arr: string): string {
  return `(SELECT string_agg((e->>'name') || '|' || (e->>'team') || '|' || (e->>'season'), ','
...
export async function getUserTeamsRO(
  ...
  const rows = await queryTournamentRO<TeamSummaryRow>(
    `SELECT t.team_id::text AS team_id, t.team_name, t.mode,
     ...
     SELECT 'daily:' || dr.daily_date AS team_id,
            ...
            'daily' AS mode,
            0 AS record_w, 0 AS record_l, CAST(0 AS double precision) AS realized_margin,
            0 AS reached_round, '' AS champion_name,
            dr.margin AS seed_net, dr.daily_date AS daily_date, dr.created_at AS created_at,
            jsonb_build_object('roster', dr.roster_json) AS roster_display,
